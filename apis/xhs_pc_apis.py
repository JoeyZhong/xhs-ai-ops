...
class XHS_Apis():
    ...
    def search_note(self, query: str, cookies_str: str, page=1, sort_type_choice=0, note_type=0, note_time=0, note_range=0, pos_distance=0, geo="", proxies: dict = None):
        ...
        try:
            ...
            res_json = response.json()
            # 用 .get() 容错：风控软限流时响应里没有 "msg" 键（旧代码 res_json["msg"]
            # 会抛 KeyError，被误报为失败、还触发浏览器兜底，掩盖真实状态）。
            success = res_json.get("success", False)
            msg = res_json.get("msg", "")
            # 软限流识别：XHS 会返回 success=true 但 data={}（连 items 键都没有），伪装成
            # "成功的空结果"。真实空搜索是 data.items=[]（有 items 键）。保持 success=True
            # （本就是合法响应，让上层干净拿到空结果、不触发浏览器兜底——限流期再加载只会更糟），
            # 但打 warning，让操作者在日志里一眼区分"软限流" vs "真没帖"。
            _data = res_json.get("data")
            if success and isinstance(_data, dict) and "items" not in _data:
                msg = "soft-throttled: 成功响应但 data 为空（疑似风控软限流，歇几小时再扫，勿换 IP）"
                logger.warning(f"[XHS] 搜索疑似风控软限流：keyword={query} page={page}（success 但空 data，不做浏览器兜底）")
        except Exception as e:
            success = False
            msg = str(e)
        return success, msg, res_json
